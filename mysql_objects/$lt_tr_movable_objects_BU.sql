...
-- Exportiere Struktur von Trigger lif_1.$lt_tr_movable_objects_BU
DROP TRIGGER IF EXISTS `$lt_tr_movable_objects_BU`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `$lt_tr_movable_objects_BU` BEFORE UPDATE ON `movable_objects` FOR EACH ROW BEGIN
   ...
   if (new.Durability <> old.Durability or new.CreatedDurability <> old.CreatedDurability) then
      ...
      if l_parentID in (64,65,69,70,71,454) or l_grandParentID in (64,65,69,70,71,454) then
         /*
			-- log original value
		   insert into $lt_t_mov_obj_dur_log 
